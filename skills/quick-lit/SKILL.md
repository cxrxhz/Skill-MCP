---
name: quick-lit
description: "快速学术检索与文献来源检索。Use when user says \"检索文献\", \"文献检索\", \"查文献\", \"找文献\", \"检索论文\", \"找论文\", \"搜论文\", \"相关工作\", \"相关文献\", \"文献来源\", \"论文来源\", \"参考文献来源\", \"给这个说法找文献\", \"给这个结论找来源\", \"这篇文章讲了什么\", \"查参数\", \"查比热\", \"查热导率\", \"查密度\", \"物性参数\", \"检索物性\", \"材料参数来源\", \"比热需要多方文献来源\", \"需要文献确定数值\", or wants quick academic paper search, source tracing, related work mapping, paper summary with traceable citations, or specific material property values with literature sources. 注意：如果用户说\"深入研究\"、\"深入核查\"、\"深入核实\"、\"深入检索\"、\"深度调研\"、\"deep research\"，请改用 deep-research skill skill。本 skill 只做快速检索，不做对抗审阅。"
...
> ⚠️ **路由提示**: 如果用户的指令包含"深入"、"深度"、"deep"等关键词，应改为调用 deep-research skill。本 skill 定位为**快速轻量检索**。

## 核心原则

1. 能用 research-lit skill 完成就直接委派，不自建搜索链路。
...
5. **确认阶段**：当初筛锁定了关键文献但摘要信息不足以回答用户问题时，**可以读取可访问的 PDF/论文全文 深入读取**（调用 research-lit skill 的 `arXiv full-text reading when accessible` 或通过 web page retrieval capability 抓取全文页面）。
...
### 1. 判定任务 → 委派 research-lit skill

根据 `$ARGUMENTS` 判断类型，直接委派：

- **单篇论文理解**（给了链接/DOI/标题）→ research-lit skill "$ARGUMENTS" — sources: web, deepxiv`
- **来源追溯 / 主题综述 / 相关工作** → research-lit skill "$ARGUMENTS" — sources: web, semantic-scholar`
- **物性参数数值检索** → 见下方专项流程
...
1. **arXiv 论文**：调用 research-lit skill "$ARGUMENTS" — arXiv full-text reading when accessible`，下载后读取前 5-10 页
2. **非 arXiv 论文**：尝试 web page retrieval capability 抓取全文 HTML 页面（ScienceDirect、PMC、期刊开放获取页面）
...
```
第一轮: research-lit skill "Cr2Te3 specific heat density thermal properties" — sources: web, semantic-scholar
→ 命中密度 6.66 g/cm³ (crystallographic data), 比热未直接给出
第二轮: research-lit skill "Cr2Te3 heat capacity Dulong-Petit Debye" — sources: web, semantic-scholar
...
- 已完成 3 轮搜索仍只有间接线索 → 停止
- **不为读 PDF 临时配新环境**（但可以用已有工具下载/web page retrieval capability）
- **绝不调用不存在的子代理**
