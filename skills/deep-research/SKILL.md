...
> ⚠️ **路由提示**: 本 skill 仅在用户明确要求"深入/深度/deep"时调用。简单查文献请用 `/quick-lit`。

## 与 `/quick-lit` 的区别

| 维度 | `/quick-lit` | `/deep-research` |
|------|-------------|-----------------|
| 定位 | 快速检索，即问即答 | 深度调研，结构化报告 |
| PDF 阅读 | 仅在摘要不够时读 1-3 篇 | 主动下载并精读 5-10 篇关键文献 |
| 搜索轮数 | 最多 3 轮 | 最多 5 轮，含补充轮 |
| 交叉验证 | 无 | 多源交叉验证（PubMed/S2/arXiv/原始论文） |
| 对抗审阅 | 无 | GPT-5.4 xhigh 对抗式审阅 |
...
Phase 3: 声明提取与交叉验证 (每条关键声明追溯原始来源)
    ↓
Phase 4: 对抗审阅 (GPT-5.4 xhigh 扮演 devil's advocate)
    ↓
Phase 5: 结构化报告
```
...
调用 `/research-lit` 进行广域扫描：

```
/research-lit "$ARGUMENTS" — sources: web, semantic-scholar, deepxiv — arxiv download: true
```
...
```
/research-lit "补充关键词" — sources: web, semantic-scholar
```
...
2. PMC 全文（通过 WebFetch 抓取 PMC HTML）
3. 期刊开放获取页面（通过 WebFetch）
...
### 4.2 外部对抗审阅（Codex MCP）

将 Phase 2-3 的所有发现提交给 GPT-5.4 xhigh 进行对抗式审阅：
...
使用 `mcp__codex__codex-reply` 对审阅意见进行反驳/吸收：
- 对合理批评：修正分析
- 对不合理批评：提供反驳证据
- 直到双方在核心结论上达成共识

### 4.4 无 Codex MCP 时的降级方案
...
- **对抗审阅是必选项**：不是可跳过的步骤。如 Codex 不可用，用内部子代理降级执行。
- **省流版是必选项**：报告开头必须有 3-5 句话的核心结论，方便用户先说清楚再展开。
- **Large file handling**: 如果 Write tool 因文件过大失败，用 Bash (`cat << 'EOF' > file`) 分块写入，不需要问用户。
